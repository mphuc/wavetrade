...
function get_date_ico(callback){
	...
	var d = new Date(year, month, day, '10', '00', '00', '00');
	callback(d);
}
...
function get_price_coin(req, res){

	Ticker.findOne({},(err,data)=>{
		Volume.findOne({'MarketName' : 'BTC-SFCC'},(err,data_ticker)=>{
			return res.status(200).send((parseFloat(data.btc.usd)*(parseFloat(data_ticker.last)/100000000)).toFixed(8))
		});
	})
}
